...
class CodeGenerator:
    ...
    def format_arg(self, key: str, value: any) -> str:
        ...
        """
        # Sanitize parameter names that contain spaces or other invalid characters
        # Check if key is a valid Python identifier (starts with letter/underscore, contains only alphanumeric/underscore)
        if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', key):
            ...
            use_key = sanitized_key
        else:
            use_key = key
        
        if key == "noise_seed" or key == "seed":
            return f"{use_key}=random.randint(1, 2**64)"
        elif isinstance(value, str):
            value = value.replace("\n", "\\n").replace('"', "'")
            return f'{use_key}="{value}"'
        elif isinstance(value, dict) and "variable_name" in value:
            return f'{use_key}={value["variable_name"]}'
        return f"{use_key}={value}"
